...
import { 
  Calculator, DollarSign, CreditCard, Briefcase, Home,
...
const PreQualificationCalculator = () => {
  ...
  const [result, setResult] = useState<{
    maxLoan: number; monthlyPayment: number; dti: number; score: number;
    status: 'excellent' | 'good' | 'fair' | 'needs_work';
  } | null>(null);
  const [contactErrors, setContactErrors] = useState<Record<string, string>>({});
  
  const [formData, setFormData] = useState<PreQualData>({
    annualIncome: '', monthlyDebts: '', creditScore: '700', downPayment: '',
  ...
  const calculatePreQual = () => {
    ...
    let score = 50;
    if (credit >= 740) score += 25; else if (credit >= 700) score += 20; else if (credit >= 660) score += 10;
    if (dti <= 36) score += 25; else if (dti <= 43) score += 15;
    if (downPayment >= maxHomePrice * 0.20) score += 10; else if (downPayment >= maxHomePrice * 0.10) score += 5;
    const status = score >= 80 ? 'excellent' : score >= 60 ? 'good' : score >= 40 ? 'fair' : 'needs_work';
    setResult({ maxLoan: Math.max(0, maxHomePrice), monthlyPayment: Math.max(0, availableForMortgage), dti: Math.min(100, Math.max(0, dti)), score, status });
  };
  ...
  const handlePhoneVerified = async () => {
    ...
    try {
      ...
      setShowOTP(false);
      setStep(5);
      toast({ title: "Pre-qualification complete!", description: "A mortgage specialist will contact you soon." });
    } catch (error) {
    ...
    switch (step) {
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              <div className="w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center mx-auto mb-3"><DollarSign className="w-6 h-6 text-accent" /></div>
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <Input id="annualIncome" name="annualIncome" type="number" placeholder="75,000" value={formData.annualIncome} onChange={handleChange} className="pl-8" />
              </div>
            </div>
          </div>
        );
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              <div className="w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center mx-auto mb-3"><CreditCard className="w-6 h-6 text-accent" /></div>
            ...
            <div>
              ...
              <div className="relative mt-1">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                <Input id="monthlyDebts" name="monthlyDebts" type="number" placeholder="500" value={formData.monthlyDebts} onChange={handleChange} className="pl-8" />
              </div>
            ...
            <div>
              ...
              <select id="creditScore" name="creditScore" value={formData.creditScore} onChange={handleChange} className="w-full mt-1 px-3 py-2 border border-input rounded-md bg-background">
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              <div className="w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center mx-auto mb-3"><Home className="w-6 h-6 text-accent" /></div>
            ...
            <div>
              ...
              <div className="relative mt-1">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">$</span>
                <Input id="downPayment" name="downPayment" type="number" placeholder="20,000" value={formData.downPayment} onChange={handleChange} className="pl-8" />
              </div>
            </div>
          </div>
        );
      case 4:
        return (
          <div className="space-y-4">
            <div className="text-center mb-6">
              <div className="w-12 h-12 bg-accent/20 rounded-full flex items-center justify-center mx-auto mb-3"><Briefcase className="w-6 h-6 text-accent" /></div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="firstName" name="firstName" placeholder="John" value={formData.firstName} onChange={handleChange} className={contactErrors.firstName ? 'border-destructive' : ''} />
                {contactErrors.firstName && <p className="text-xs text-destructive mt-1">{contactErrors.firstName}</p>}
              </div>
              <div>
                <Label htmlFor="lastName">Last Name *</Label>
                <Input id="lastName" name="lastName" placeholder="Doe" value={formData.lastName} onChange={handleChange} className={contactErrors.lastName ? 'border-destructive' : ''} />
                {contactErrors.lastName && <p className="text-xs text-destructive mt-1">{contactErrors.lastName}</p>}
              </div>
            </div>
            <div>
              <Label htmlFor="email">Email *</Label>
              <Input id="email" name="email" type="email" placeholder="john@example.com" value={formData.email} onChange={handleChange} className={contactErrors.email ? 'border-destructive' : ''} />
            ...
            {contactErrors.consent && <p className="text-xs text-destructive">{contactErrors.consent}</p>}
          </div>
        );
      case 5:
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${result.status === 'excellent' ? 'bg-green-100' : result.status === 'good' ? 'bg-blue-100' : result.status === 'fair' ? 'bg-yellow-100' : 'bg-red-100'}`}>
                {result.status === 'excellent' || result.status === 'good'
                  ? <CheckCircle className={`w-8 h-8 ${result.status === 'excellent' ? 'text-green-600' : 'text-blue-600'}`} />
                  : <AlertCircle className={`w-8 h-8 ${result.status === 'fair' ? 'text-yellow-600' : 'text-red-600'}`} />}
              </div>
              <h3 className="text-2xl font-bold text-foreground mb-2">
                {result.status === 'excellent' ? 'Excellent!' : result.status === 'good' ? 'Looking Good!' : result.status === 'fair' ? 'Getting There' : "Let's Work Together"}
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="p-4 bg-muted/50">
                ...
                <p className="text-2xl font-bold text-foreground">${result.maxLoan.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
              </Card>
              <Card className="p-4 bg-muted/50">
                <p className="text-sm text-muted-foreground">Est. Monthly Payment</p>
                <p className="text-2xl font-bold text-foreground">${result.monthlyPayment.toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
              </Card>
            </div>
            ...
            <p className="text-sm text-muted-foreground text-center">A mortgage specialist from Previse Mortgage will contact you within 24 hours to discuss your personalized options.</p>
            <Button className="w-full" onClick={() => window.location.href = '/schedule'}>
              Schedule a Call Now <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </div>
        );
      default: return null;
