...
const MortgageCalculator = () => {
  ...
  const [refinanceSavings, setRefinanceSavings] = useState<{
    monthly: number;
    total: number;
  } | null>(null);
  ...
  const [dscrResult, setDscrResult] = useState<{
    ratio: number;
    monthlyDebt: number;
    status: string;
  } | null>(null);
  ...
  const calculateRefinanceSavings = () => {
    ...
    if (balance > 0 && oldRate > 0 && newRateMonthly > 0 && months > 0) {
      const oldPayment = balance * oldRate * Math.pow(1 + oldRate, months) / (Math.pow(1 + oldRate, months) - 1);
      const newPayment = balance * newRateMonthly * Math.pow(1 + newRateMonthly, months) / (Math.pow(1 + newRateMonthly, months) - 1);
  ...
  const calculateAffordability = () => {
    ...
    if (income > 0) {
      ...
      const months = 360;
      const loanAmount = maxMonthlyPayment > 0 ? maxMonthlyPayment * (Math.pow(1 + estimatedRate, months) - 1) / (estimatedRate * Math.pow(1 + estimatedRate, months)) : 0;
  ...
  const calculateDSCR = () => {
    ...
    if (rent > 0 && loanAmt > 0 && rate > 0 && term > 0) {
      ...
      const piPayment = loanAmt * rate * Math.pow(1 + rate, term) / (Math.pow(1 + rate, term) - 1);
  ...
  return <div className="min-h-screen bg-gradient-to-b from-primary-dark via-primary to-primary-dark">
      <Header />
      
      {/* Back to Home */}
      <div className="container mx-auto px-4 pt-6">
        <Button variant="ghost" onClick={() => navigate(-1)} className="text-white hover:text-accent">
      ...
      <main className="pt-8 pb-16">
        ...
        <section className="py-12 relative overflow-hidden">
          <div className="absolute inset-0">
            ...
            <div className="absolute bottom-1/4 left-1/4 w-64 h-64 bg-accent/10 rounded-full blur-2xl animate-pulse-slow" style={{
            animationDelay: '1.5s'
          }}></div>
          </div>
            ...
            <div className="max-w-4xl mx-auto">
              <Tabs defaultValue="monthly" className="w-full">
                <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 bg-white/5 border border-white/10 rounded-2xl p-2 mb-8 h-auto gap-2">
                  <TabsTrigger value="monthly" className="data-[state=active]:bg-accent data-[state=active]:text-primary-dark rounded-xl py-3 text-white/70 justify-center text-xs md:text-sm">
                    <Calculator className="w-4 h-4 mr-1 md:mr-2" />
                    <span className="hidden sm:inline">Monthly </span> Payment
                  </TabsTrigger>
                  <TabsTrigger value="refinance" className="data-[state=active]:bg-accent data-[state=active]:text-primary-dark rounded-xl py-3 text-white/70 justify-center text-xs md:text-sm">
                    <RefreshCw className="w-4 h-4 mr-1 md:mr-2" />
                    Refinance
                  </TabsTrigger>
                  <TabsTrigger value="affordability" className="data-[state=active]:bg-accent data-[state=active]:text-primary-dark rounded-xl py-3 text-white/70 justify-center text-xs md:text-sm">
                    <Home className="w-4 h-4 mr-1 md:mr-2" />
                    Affordability
                  </TabsTrigger>
                  <TabsTrigger value="dscr" className="data-[state=active]:bg-accent data-[state=active]:text-primary-dark rounded-xl py-3 text-white/70 justify-center text-xs md:text-sm">
                ...
                <TabsContent value="monthly">
                  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <Input type="number" placeholder="400,000" value={homePrice} onChange={e => setHomePrice(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="80,000" value={downPayment} onChange={e => setDownPayment(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Interest Rate (%)</Label>
                        <Input type="number" step="0.1" placeholder="7.0" value={interestRate} onChange={e => setInterestRate(e.target.value)} className="bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Loan Term (Years)</Label>
                        <select value={loanTerm} onChange={e => setLoanTerm(e.target.value)} className="w-full h-10 px-3 rounded-md bg-white/5 border border-white/20 text-white">
                    ...
                    {monthlyPayment !== null && <div className={`${monthlyPayment > 0 ? 'bg-accent/10 border-accent/30' : 'bg-orange-500/10 border-orange-500/30'} border rounded-2xl p-6 text-center`}>
                  ...
                  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="300,000" value={currentLoanBalance} onChange={e => setCurrentLoanBalance(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Current Interest Rate (%)</Label>
                        <Input type="number" step="0.1" placeholder="7.5" value={currentRate} onChange={e => setCurrentRate(e.target.value)} className="bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">New Interest Rate (%)</Label>
                        <Input type="number" step="0.1" placeholder="6.5" value={newRate} onChange={e => setNewRate(e.target.value)} className="bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Remaining Years on Loan</Label>
                        <Input type="number" placeholder="25" value={remainingYears} onChange={e => setRemainingYears(e.target.value)} className="bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                      </div>
                    </div>
                    ...
                    {refinanceSavings && (refinanceSavings.monthly > 0 ? <div className="grid md:grid-cols-2 gap-4">
                        ...
                        </div> : <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-6 text-center">
                  ...
                  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="100,000" value={annualIncome} onChange={e => setAnnualIncome(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="500" value={monthlyDebts} onChange={e => setMonthlyDebts(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div className="md:col-span-2">
                        <Label className="text-white/80 mb-2 block">Down Payment Percentage (%)</Label>
                        <select value={downPaymentPercent} onChange={e => setDownPaymentPercent(e.target.value)} className="w-full h-10 px-3 rounded-md bg-white/5 border border-white/20 text-white">
                    ...
                    {affordability && (affordability.maxHome > 0 && affordability.maxPayment > 0 ? <div className="grid md:grid-cols-2 gap-4">
                          <div className="bg-accent/10 border border-accent/30 rounded-2xl p-6 text-center">
                  ...
                  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="2,500" value={monthlyRent} onChange={e => setMonthlyRent(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="300,000" value={dscrLoanAmount} onChange={e => setDscrLoanAmount(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Interest Rate (%)</Label>
                        <Input type="number" step="0.1" placeholder="7.5" value={dscrInterestRate} onChange={e => setDscrInterestRate(e.target.value)} className="bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                      </div>
                      <div>
                        <Label className="text-white/80 mb-2 block">Loan Term (Years)</Label>
                        <select value={dscrLoanTerm} onChange={e => setDscrLoanTerm(e.target.value)} className="w-full h-10 px-3 rounded-md bg-white/5 border border-white/20 text-white">
                          <option value="15" className="bg-primary-dark">15 Years</option>
                          <option value="20" className="bg-primary-dark">20 Years</option>
                          <option value="30" className="bg-primary-dark">30 Years</option>
                        </select>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="3,600" value={propertyTaxes} onChange={e => setPropertyTaxes(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="1,800" value={insurance} onChange={e => setInsurance(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                      <div className="md:col-span-2">
                        ...
                        <div className="relative">
                          <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                          <Input type="number" placeholder="0" value={hoaFees} onChange={e => setHoaFees(e.target.value)} className="pl-9 bg-white/5 border-white/20 text-white placeholder:text-white/40" />
                        </div>
                      </div>
                    </div>
                    ...
                    {dscrResult && <div className="space-y-4">
                        <div className="bg-accent/10 border border-accent/30 rounded-2xl p-6 text-center">
              ...
              <div className="mt-12 bg-gradient-to-r from-accent/20 to-accent/10 border border-accent/30 rounded-3xl p-8 text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <a href="https://previsemortgage.my1003app.com?time=1766329396091" target="_blank" rel="noopener noreferrer">
